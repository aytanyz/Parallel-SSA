...
void rejectionSSA(int i_th, int num_partitions, int num_update, int sub_domain[M])
{
    ...
    double temp_time = chrono::duration_cast<chrono::microseconds>(elapsedReac).count();
...
int main(int argc, char * argv[])
{
    ...
    cout<<"Total_time: " << chrono::duration_cast<chrono::microseconds>(elapsedReac).count() << " microseconds"<< endl<<endl; 
    cout<<"overall_iter: "<<overall_iter<<endl;
    cout<<"overall_real_iter: "<<overall_real_iter<<endl;
    cout<<"Overall_x: ";
    for(int i=0; i<M; i++)
        cout<<" "<<overall_x[i];
    cout<<endl<<"Overall_time: "<<overall_time << " microseconds"<< endl<<endl; 
    return 0;
}